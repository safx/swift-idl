...
public enum JSONDecodeError: ErrorType, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .MissingKey(let v): return "MissingKey(key=\(v.key), object=\(v.object))"
        case .TypeMismatch(let v): return "TypeMismatch(key=\(v.key), object=\(v.object), expected=\(v.expected), actual=\(v.actual))"
        case .ValueTranslationFailed(let v): return "ValueTranslationFailed(type=\(v.type), object=\(v.object))"
        case .NonNullable(let v): return "NonNullable(key=\(v.key), object=\(v.object))"
        }
    }
}
